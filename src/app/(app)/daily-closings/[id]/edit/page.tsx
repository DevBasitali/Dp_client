...
import { useDailyClosings, useUpdateDailyClosing, DailyClosing } from "@/hooks/useDailyClosings";
...
function EditDailyClosingForm({ closing }: { closing: DailyClosing }) {
  const router = useRouter();
  const { user } = useAuthStore();
  const isManager = user?.role === "branch_manager";

  const { data: branches } = useBranches();
  const updateMutation = useUpdateDailyClosing();
  ...
  const {
    ...
    defaultValues: {
      ...
      closingDate: closing.closingDate ? format(new Date(closing.closingDate), "yyyy-MM-dd") : format(new Date(), "yyyy-MM-dd"),
      cashSales: Number(closing.cashSales) || 0,
      easypaisaSales: Number(closing.easypaisaSales) || 0,
      notes: closing.notes || "",
      expenses: closing.expenses?.map(e => ({
        description: e.description,
        amount: Number(e.amount),
        source: e.source as "SALE" | "CAL"
      })) || [],
    },
  });
    ...
    updateMutation.mutate(payload, {
      ...
      onError: (err: any) => {
        ...
        if (status === 409) {
          ...
          toast.error(msg || "Failed to update. Please try again.");
        }
      },
    });
  };
  ...
  return (
    <div className="space-y-6 lg:p-4 max-w-4xl mx-auto pb-28">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-[#1A1A2E]">Edit Daily Closing</h1>
          <p className="text-gray-500 text-sm">Update end-of-day sales and expenses.</p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {!isManager && (
                <div className="space-y-2">
                  ...
                  <Select
                    defaultValue={closing.branchId}
                    onValueChange={(val: string | null) => val && setValue("branchId", val)}
                  >
              ...
              <div className="space-y-2">
                <Label>Closing Date <span className="text-gray-500">(Locked)</span></Label>
                <Input
                  type="date"
                  disabled
          ...
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            ...
            >
              {updateMutation.isPending ? (
                ...
                <><Save className="w-5 h-5 mr-2" />Update Daily Closing</>
              )}
            </Button>
          </div>
        </div>
...
export default function EditDailyClosingPageWrapper({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const { data: closings, isLoading, isError } = useDailyClosings();
  const router = useRouter();
  ...
  const closing = closings.find((c) => c.id === resolvedParams.id);

  if (!closing) {
    return (
      <div className="flex flex-col justify-center items-center h-48 space-y-4">
        <p className="text-gray-500">Daily closing not found.</p>
        <Button onClick={() => router.back()} variant="outline">Go Back</Button>
      </div>
    );
  }

  return <EditDailyClosingForm closing={closing} />;
}
